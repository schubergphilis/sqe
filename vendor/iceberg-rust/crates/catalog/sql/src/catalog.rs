...
impl SqlCatalog {
    ...
    async fn new(
        ...
        // string, not a StorageFactory. The factory parameter is accepted
        // for API compatibility but unused on the fork. Default to "s3"
        // since SQL catalog deployments commonly point at S3-backed
        // warehouses; users with other backends can override via props.
        let _ = storage_factory;
        let scheme: String = config
            .warehouse_location
            .split("://")
            .next()
            .filter(|s: &&str| !s.is_empty() && *s != config.warehouse_location.as_str())
            .unwrap_or("s3")
            .to_string();
        let fileio = FileIOBuilder::new(scheme).build()?;
