...
//! Snapshot history traversal helpers.
//!
//! This module lives at `util::snapshot` to match apache iceberg-rust's
//! organization (apache PR #2285).  Our pre-existing implementation
//! returned `Box<dyn Iterator<...>>` so the iterators could be
//! conditionally swapped for `std::iter::empty()` when the start snapshot
//! is missing; apache prefers `impl Iterator<...>` with an empty
//! `Ancestors` representing the missing case.  Both shapes are
//! semantically identical for callers that just iterate.

use crate::spec::{SnapshotRef, TableMetadataRef};

/// Iterator over the chain of snapshots reachable from a starting
/// snapshot by following `parent_snapshot_id` until the root.
...
/// Iterate starting from `snapshot` (inclusive) to `oldest_snapshot_id`
/// (exclusive).  When `oldest_snapshot_id` is `None` this is equivalent
/// to [`ancestors_of`].
