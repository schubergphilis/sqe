...
mod tests {
    ...
    async fn test_retry_marks_worker_unhealthy() {
        // When a worker fails during execution, it should be marked unhealthy
        // in the registry.
        ...
        .with_max_retries(0); // No retries — just verify the mark

        let context = Arc::new(TaskContext::default());
        let stream = exec.execute(0, context).unwrap();

        // Collect the stream — it will fail because the worker isn't real
    ...
    async fn test_local_fallback_when_all_workers_fail() {
        // When all workers fail, the local executor should be invoked.
        let registry = Arc::new(WorkerRegistry::new(vec![
            "http://w1:50052".to_string(),
        ]));
        registry.mark_healthy("http://w1:50052").await;
        ...
        .with_worker_registry(registry.clone())
        .with_max_retries(0) // No retries — go straight to fallback
        ...
        // The local executor should have been called
        assert_eq!(local_exec.calls(), 1, "Local executor should be called once");

        // The result should contain our marker batch
    ...
    async fn test_no_fallback_returns_error() {
        // When workers fail and no local fallback is configured, the error propagates.
        let registry = Arc::new(WorkerRegistry::new(vec![
            "http://w1:50052".to_string(),
        ]));
        registry.mark_healthy("http://w1:50052").await;
    ...
    async fn test_retry_tries_different_worker() {
        // With 2 workers and max_retries=1, after the first worker fails,
        // the retry should attempt the second worker. Both will fail
        // (unreachable), but we verify both get marked unhealthy.
        let registry = Arc::new(WorkerRegistry::new(vec![
            "http://w1:50052".to_string(),
            "http://w2:50052".to_string(),
        ]));
        ...
        .with_worker_registry(registry.clone())
        .with_max_retries(1); // One retry allowed
        ...
        // Both workers should be marked unhealthy
        // (w1 failed on first attempt, w2 failed on retry)
        assert!(
            registry.healthy_workers().await.is_empty(),
    ...
    async fn test_local_fallback_after_retries_exhausted() {
        // All workers fail, retries exhaust, then local fallback kicks in.
        let registry = Arc::new(WorkerRegistry::new(vec![
            "http://w1:50052".to_string(),
            "http://w2:50052".to_string(),
        ]));
        ...
        // Local executor should be called after both workers fail
