-- name: Channel net profit comparison with and without promotions
-- timeout: 60s
WITH ssr AS (
...
FROM (
    SELECT 'store channel' AS channel, store_id AS id, sales, returns, profit FROM ssr
    UNION ALL
    SELECT 'catalog channel', catalog_page_id, sales, returns, profit FROM csr
    UNION ALL
    SELECT 'web channel', web_site_id, sales, returns, profit FROM wsr
) x
GROUP BY channel, id
ORDER BY channel, id
LIMIT 100;
