...
2. **List comprehensions and the remaining lambdas** (`reduce`, `all_match`, `none_match`): no DataFusion primitive. The `filter` and `transform` lambdas do work: SQE parses with the DuckDB dialect and aliases them onto DataFusion 54's `array_filter` / `array_transform`.
