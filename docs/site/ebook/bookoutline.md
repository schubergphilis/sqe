...
| 8b | **A GRANT That Polaris Enforces** | 14 | The catalog-based access-control backend. SQL `GRANT`/`REVOKE` becomes Apache Ranger policy; Polaris 1.5's embedded authorizer enforces; SQE does no filtering. The hard-won identity model: principals must pre-exist in Polaris (the token is a lookup key, not an identity source), roles come from Ranger membership not token realm roles, `root="*"` or grants silently never match, one privilege expands to the full explicit access-type set, the LOAD_TABLE read gate. Deny by omission survives the change of enforcer. |
| 8c | **One Policy, Two Engines** | 16 | Ranger as a third fine-grained backend alongside OPA/Cedar. SQE rewrites the plan against the shared `hive` service: row filters, the full Hive mask vocabulary, role-conditional masking via const-folded session UDFs (Snowflake parity), fail-closed throughout. Tag rule in Ranger `tagPolicies`, tag-to-column association in the Iceberg `sqe.column-tags` property. The kicker: the same policy enforces byte-exact in standard Spark (validated live, `xxx-xx-1111` in both), with the honest edges (tags not cross-compared, Spark 4 not feasible, `catalogImplementation=hive` required). |
...
| 16d | **The DuckDB Drift** | 12 | Five MRs (V8 through V12) added file-format TVFs, a lazy HTTPS object store, a HuggingFace `hf://` resolver, a Delta Lake reader, and a smart CSV parser. None of them were planned in the original roadmap; each came from a single user request. The audit table that found two false negatives (features SQE already had but did not document) and eight genuine gaps that became the V8-V12 plan. Why convergence between distributed SQL engines and embedded analytics is the default when the building blocks (DataFusion, iceberg-rust, object_store) are good enough. What we did not become and where we drew the line. |
| 16f | **The Maintenance Exception** | 14 | Autonomous compaction versus the no-service-account rule: the one background actor with no human behind it. A maintenance service principal that is structurally unreachable from the interactive auth chain, deny-by-default, opt-in per table on both sides (owner property and Polaris grant). Delete-aware rewrite, the sequence-number pin, and sort producing disjoint prunable key ranges. Distributing the rewrite to workers while commit authority stays on the coordinator. The fast-append lease that gave no exclusion, and the `check_file_existence` swap that does. Why the catalog lease is an efficiency layer, not where correctness lives. What shipped verified by construction and what still waits on a real multi-worker benchmark. |
