...
The getsqe Astro app renders these files via the content sync: prose lives in the `.md` files, and the structured lists and tables live in the `.yaml` files. Keep the wording faithful and follow the repo style guide (no emdash, endash, or unicode arrows; placeholders such as account id `123456789012` stay as placeholders). HTML tags and entities inside the YAML values are intentional and the Astro renderer consumes them, leave them as-is.
