...
- **Attached golden tables for the read suites**: `scripts/benchmark-publish-iceberg.sh` publishes tpch/ssb/tpcds/tpcbb/clickbench/bank into a persistent Polaris once; `BENCH_DATA_SOURCE=attach` then runs those six read-only suites against the attached `golden` catalog with zero per-run load. Write suites (tpcc/tpce) still generate and load normally; a shallow-clone path that lets them share golden data files while writing locally is the deferred follow-up. See [Benchmark Suite](../features/benchmarks.md#fast-benchmark-runs-via-attached-golden-tables).
