...
| Renaming a tagged column orphans the tag | **open** | `sqe.column-tags` is keyed by column NAME and no schema-change path rewrites it. After `RENAME COLUMN ssn TO tax_id` the mask stops applying, in BOTH engines. |
