...
| Tag masks without the projector | **open** until projected | Associations live in the Iceberg property, which Kyuubi cannot read, so a column masked in SQE comes back raw in Spark. Closed for new tags by `project-tags = true` on `SET TAG`. Tables tagged before the projector existed stay SQE-only until an admin runs `CALL system.reproject_column_tags(table => 'ns.t')` (or `namespace` / `catalog`). |
