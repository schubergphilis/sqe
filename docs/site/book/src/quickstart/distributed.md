...
`./run.sh --check` asserts the distributed invariants end to end: connectivity over Flight SQL, `system.runtime.nodes` lists the coordinator, `system.runtime.queries` records `FINISHED` history, a CTAS round-trip and read-back, cache invalidation on write, the Trino HTTP endpoint answers, and the distributed-execution invariant itself: `system.runtime.tasks` shows fragments dispatched to `worker-1` / `worker-2` after a query that forces a split, not just the coordinator.
