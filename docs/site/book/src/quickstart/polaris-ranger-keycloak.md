...
**Session-context functions (shipped).** `current_user()`, `current_role()`, and
`is_role_in_session()` resolve inside row-filter expressions, const-folded per session so a
distributed plan stays correct. A richer role model for inherited and secondary roles is a future
refinement.

**Tag-based masking (shipped).** A mask-per-tag rule lives in the Ranger `tag`-service policy; the
tag-to-column association is authored with `ALTER TABLE ... SET TAGS` and stored in the Iceberg
`sqe.column-tags` property. See [Fine-grained access control](../features/fine-grained-access-control.md#tag-based-masking).
