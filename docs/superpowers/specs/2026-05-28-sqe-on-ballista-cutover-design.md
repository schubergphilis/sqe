...
| 2 | **Policy plans survive the codec** — injected column-mask / row-filter nodes round-trip through `SqeLogicalCodec` and enforce on executors | **GAP FOUND + FIXED, code-complete (2026-05-31).** Assessed: row filters (standard `Filter` exprs) and column restriction (projection) survive via ballista's default codec; **column masks did NOT** — the mask UDF (`sha256`) was registered on neither the scheduler (planning) nor the executor (run), so a masked plan failed to resolve `sha256`. Same gap hit JSON funcs + Trino-function *execution*. Fix (commit 2a06257): shared `register_sqe_session_udfs` (sha256 + Trino + extended-Trino + JSON) wired into coordinator + scheduler `session_builder` + executor `override_function_registry`. Verified: unit test (helper registers `sha256` beyond defaults, flows into `BallistaFunctionRegistry`); distributed stack stood up (coordinator scheduler + rebuilt `sqe-worker` executor) and the executor **starts and registers the `sha256` UDF** (worker log) with no crash from `override_function_registry`. NOT yet shown: a `sha256` query returning rows E2E — blocked by **client-auth tooling**, not the fix: `sqe-cli` needs an interactive TTY for the password and rejects bearer tokens; ADBC FlightSQL's standard `AuthenticateBasicToken` handshake is rejected by the coordinator's SQE-specific handshake ("Invalid base64 in auth"). The proven-working non-interactive client (sqe-bench, Rust) runs only fixed suites. A human with a TTY can confirm via `sqe-cli --user root -e "SELECT sha256(...)"`. (Side-finding worth a separate look: the ADBC handshake incompatibility may affect the dbt-sqe ADBC adapter.) Row-filter *enforcement* E2E also not stood up (needs a policy backend). | G3 |
