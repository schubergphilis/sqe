# Building a Comprehensive SQL Benchmark Suite for SQE
...
**TPC-C and TPC-E** cover OLTP read patterns — point lookups, small aggregates, indexed access by key. Their write queries (new order, payment, trade order) are annotated with `-- requires: delete, merge` and are skipped until SQE's DELETE/MERGE support lands. This is important: we wanted the OLTP schemas and read queries exercised now, not after a feature gate is cleared.
...
**Load** connects to SQE and creates Iceberg tables from the generated Parquet files. This is where the `read_parquet()` TVF becomes essential. Rather than defining an intermediate import format or writing a custom loader, load simply sends one CTAS statement per table:
...
This makes `sqe-bench load` self-contained: it does not depend on environment variables, instance profiles, or credential files on the coordinator's filesystem. The credentials live in the SQL statement, which is transmitted over an authenticated Flight SQL connection. This is the same pattern that users will reach for when they need to pull data from an external bucket during a migration.
