...
> Status as of 2026-06-01 (updated). **Web UI metrics dashboard shipped.** Overview tab is now a full metrics dashboard: stat cards (total/finished/failed/running/avg latency/rows out), 12h query activity SVG histogram (completed vs failed stacked bars), and two resource gauges (memory % + concurrency %) with horizontal fill bars and sparklines. Backend: `MetricsHistory` ring-buffer (4320 samples, 10s interval, 12h window) sampled by a supervised `TaskGuard` task; `GET /api/v1/metrics/history` returns up to 48 pre-aggregated 900s buckets (`queriesCompleted`, `queriesFailed`, `avgActive`, `maxActive`, `avgMemPct`, `maxMemPct`). The original `.kv` Requirements panel remains below as config reference. `[metrics] web_ui = true` still required.
