...
# Service principals. Each Keycloak SP client mints a client_credentials token
# whose preferred_username is the SP name (hardcoded mapper, verified), so the
# Polaris principal entity must exist under that exact name or Polaris rejects
# the federated token with "Failed to resolve principal". Created as USER to
# match the demo users above (the type the Ranger authorizer federates here).
log "creating service principals (sp-admin, sp-reader, sp-denied)"
for sp in sp-admin sp-reader sp-denied; do mkprincipal "$sp"; done
