...
# GET /service/plugins/policies/download/query and rewrites the query plan
...
echo "Creating the query service instance for fine-grained policies (idempotent) ..."
if curl -fsS $AUTH "${RANGER_URL}/service/public/v2/api/service/name/query" >/dev/null 2>&1; then
  ...
  curl -fsS $AUTH $CSRF $CT -X POST "${RANGER_URL}/service/public/v2/api/service" \
    -d '{"name":"query","type":"hive","configs":{"username":"admin","password":"none","jdbc.driverClassName":"org.apache.hive.jdbc.HiveDriver","jdbc.url":"none"},"isEnabled":true}' >/dev/null \
...
echo "Creating fine-grained access + mask policies on the query service ..."
...
# ACCESS on the query service itself (it denies the query before applying any
# mask if no access policy grants select). SQE does NOT use frontend access
# policies (its coarse gate is the Polaris `polaris` service); SQE reads only
# the mask (type 1) and row-filter (type 2) policies from it and ignores
...
EOF
post_policy /tmp/query-access.json
